...
RC Parser::parse(QueryInfo *query_info)
{
  ...
  if (sql_node->flag == SCF_CREATE_TABLE) {
    for (int i = 0; i < sql_node->create_table.attr_infos.size(); i++) {
      if (sql_node->create_table.attr_infos[i].type == TEXTS) {
        sql_node->create_table.attr_infos[i].length = MAX_TEXT_VALUE_LENGTH_IN_RECORD;
      }
    }
  }
  ...
  if(sql_node->flag == SCF_SELECT) {
    for (const auto & conditionSqlNode : sql_node->selection.where_conditions.conditions) {
      ...
      if (conditionSqlNode.right_expr != nullptr && conditionSqlNode.right_expr->type() == ExprType::VALUE) {
        auto *right_expr = dynamic_cast<ValueExpr *>(conditionSqlNode.right_expr);
