...
RC GroupByStmt::create_group_by_unit(
  ...
  if (nullptr == table) {
    LOG_WARN("No such table: unit.relation_name: %s", unit.relation_name.c_str());
    return RC::SCHEMA_TABLE_NOT_EXIST;
  }

  const FieldMeta *field = table->table_meta().field(unit.attribute_name.c_str());
  if (nullptr == field) {
    LOG_WARN("no such field in table: table %s, field %s", table->name(), unit.attribute_name.c_str());
