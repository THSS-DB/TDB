...
RC CreateIndexStmt::create(Db *db, const CreateIndexSqlNode &create_index,
                           Stmt *&stmt) {
  stmt = nullptr;

  const char *table_name = create_index.relation_name.c_str();
  if (is_blank(table_name) || is_blank(create_index.index_name.c_str()) ||
      create_index.multi_attribute_names.size() == 0) {
    LOG_WARN(
        "invalid argument. db=%p, table_name=%p, index name=%s, attributes "
        "amount=%d",
        db, table_name, create_index.index_name.c_str(),
        create_index.multi_attribute_names.size());
  ...
  for (int i = 0; i < create_index.multi_attribute_names.size(); i++) {
    const FieldMeta *field_meta = table->table_meta().field(
        create_index.multi_attribute_names[i].c_str());
  ...
  stmt = new CreateIndexStmt(table, multi_field_metas, create_index.index_name,
                             create_index.is_unique_);
