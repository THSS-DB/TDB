...
InsertStmt::InsertStmt(Table *table,
                       ...
                       int record_amount, int value_amount)
    : table_(table),
      multi_values_(multi_values),
...
RC InsertStmt::create(Db *db, const InsertSqlNode &inserts, Stmt *&stmt) {
  ...
  if (nullptr == db || nullptr == table_name || inserts.multi_values.empty()) {
    LOG_WARN("invalid argument. db=%p, table_name=%p, record_num=%d", db,
             table_name, static_cast<int>(inserts.multi_values.size()));
  ...
  for (int i = 0; i < inserts.multi_values.size(); i++) {
    ...
    if (field_num != value_num) {
      if (is_view) {
        ...
        const std::vector<FieldMeta> *view_metas =
            view->table_meta().field_metas();
        const std::vector<FieldMeta> *origin_metas =
            table->table_meta().field_metas();
        ...
        for (int j = sys_field_num; j < origin_metas->size() - 1; j++) {
          ...
          for (int k = 0; k < view_metas->size(); k++) {
            if (strcmp(view_metas->at(k).name(), origin_metas->at(j).name()) ==
                0) {
              matched = true;
  ...
  for (int v = 0; v < multi_values.size(); v++) {
    std::vector<Value> &values =
        const_cast<std::vector<Value> &>(multi_values[v]);
    //  }
    ...
    for (int i = 0; i < field_num; i++) {
      ...
      if (field_type == TEXTS && value_type == CHARS) {
        ...
        std::string value_search_for_text = std::string(table->dir()) + "/" +
        ...
        values[i].set_text(value_search_for_text.c_str(),
