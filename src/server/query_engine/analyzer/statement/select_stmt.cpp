...
static void wildcard_fields_without_table_name(
    Table *table, std::vector<Expression *> &projects,
...
static void wildcard_fields_with_table_name(Table *table,
...
static void wildcard_fields(
    Table *table, int table_count, std::vector<Expression *> &projects,
...
RC _process_attribute_expression(
    Db *db, const Expression *expr, const char *table_name,
    const char *field_name, std::vector<Table *> &tables,
    std::vector<Expression *> &projects,
    std::unordered_map<Table *, std::string> alias_map,
   ...
   * This method is complex, you can try to optimize it and submit PR if you
   * like.
   */
  ...
  if (common::is_blank(table_name) &&
    ...
    if (0 == strcmp(table_name, "*")) {
      ...
      if (0 == strcmp(field_name, "*")) {  // select table.*
        ...
        field_expr->set_alias(expr->alias().empty() ? expr->name()
                                                    : expr->alias());
        projects.push_back(field_expr);
      }
    }
  } else {
    // If table_name is null, there should be only one table as default table,
...
RC SelectStmt::analyze_tables_and_projects(
  ...
  for (int i = (int)select_sql.attributes.size() - 1; i >= 0; i--) {
    ...
    // AttributeExpression needs to be processed separately because '*' in
    // attributes may cause difference.
    if (expr->type() == ExprType::REL_ATTR) {  // Simple FiledExpr
      ...
      _process_attribute_expression(db, expr,
                                    relation_attr.relation_name.c_str(),
...
RC SelectStmt::create(Db *db, const SelectSqlNode &select_sql, Stmt *&stmt) {
  ...
  RC rc = analyze_tables_and_projects(db, select_sql, tables, table_alias,
                                      table_map, projects);
  if (rc != RC::SUCCESS) {
  ...
  LOG_DEBUG("got %d tables in from stmt and %d fields in query stmt",
            tables.size(), projects.size());
  ...
  for (auto &join_list : select_sql.join_lists) {
    ...
    rc = FilterStmt::create(
        db, default_table, &table_map, join_list.join_conditions.data(),
        static_cast<int>(join_list.join_conditions.size()), join_filter_stmt);
  ...
  if (select_sql.group_by_attributes.size() > 0) {
    rc = GroupByStmt::create(db, default_table, &table_map,
                             select_sql.group_by_attributes, group_by_stmt);
    if (rc != RC::SUCCESS) {
  ...
  rc = OrderByStmt::create(
      db, default_table, &table_map, select_sql.order_lists,
      static_cast<int>(select_sql.order_lists.size()), order_stmt);
  if (rc != RC::SUCCESS) {
