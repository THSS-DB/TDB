...
RC write_to_communicator(const char *data, int32_t size,
                         Communicator *communicator, const size_t &min_width) {
  if (size < min_width) {
  ...
  switch (stmt->type()) {
    case StmtType::SELECT: {
      ...
      for (const auto *expr : select_stmt->projects()) {
        const char *alias = expr->alias().empty() ? expr->name().c_str()
                                                  : expr->alias().c_str();
...
RC send_result(SessionRequest *request, bool &need_disconnect,
  ...
  while (RC::SUCCESS == (rc = sql_result->next_tuple(tuple))) {
    ...
    for (int i = 0; i < column_num; i++) {
      ...
      rc = write_to_communicator(value_str.data(), value_str.size(),
                                 communicator, min_width);
      if (RC_FAIL(rc)) {
        LOG_WARN("failed to send data to client. err=%s", strerror(errno));
        sql_result->close();
        return rc;
...
RC Executor::execute(SessionRequest *request, QueryInfo *query_info,
