...
bool LeafIndexNodeHandler::validate(const KeyComparator &comparator, FileBufferPool *bp) const
{
...
RC InternalIndexNodeHandler::move_half_to(InternalIndexNodeHandler &other, FileBufferPool *bp)
{
  const int size = this->size();
  const int move_index = size / 2;
...
RC InternalIndexNodeHandler::move_to(InternalIndexNodeHandler &other, FileBufferPool *disk_buffer_pool)
{
...
RC InternalIndexNodeHandler::move_first_to_end(InternalIndexNodeHandler &other, FileBufferPool *disk_buffer_pool)
{
...
RC InternalIndexNodeHandler::move_last_to_front(InternalIndexNodeHandler &other, FileBufferPool *bp)
{
 ...
 */
RC InternalIndexNodeHandler::copy_from(const char *items, int num, FileBufferPool *disk_buffer_pool)
{
...
RC InternalIndexNodeHandler::append(const char *item, FileBufferPool *bp)
{
  return this->copy_from(item, 1, bp);
}

RC InternalIndexNodeHandler::preappend(const char *item, FileBufferPool *bp)
{
...
bool InternalIndexNodeHandler::validate(const KeyComparator &comparator, FileBufferPool *bp) const
{
  bool result = IndexNodeHandler::validate();
  if (false == result) {
    return false;
...
RC BplusTreeHandler::sync()
{
  if (header_dirty_) {
    Frame *frame = nullptr;
    RC rc = file_buffer_pool_->get_this_page(FIRST_INDEX_PAGE, &frame);
    if (RC_SUCC(rc) && frame != nullptr) {
                            ...
                            int leaf_max_size /* = -1 */)
{
  ...
  for (auto length: multi_attr_length) {
    total_attr_length += length;
  }
  ...
  file_header->attr_amount = multi_attr_length.size();
  file_header->attrs_length = total_attr_length;
  for (int i = 0; i < multi_attr_length.size(); i++) {
    file_header->multi_attr_lengths[i] = multi_attr_length[i];
  }
  file_header->key_length = total_attr_length + sizeof(RID);
  for (int i = 0; i < multi_attr_types.size(); i++) {
    file_header->multi_attr_types[i] = multi_attr_types[i];
  }
  ...
  file_buffer_pool_ = bp;

  if (multi_attr_length.size() > 1) {
    file_header->attrs_type = CHARS;
    key_comparator_.init(CHARS, total_attr_length);
    key_printer_.init(CHARS, total_attr_length);
  } else {
    file_header->attrs_type = multi_attr_types[0];
    key_comparator_.init(multi_attr_types[0], total_attr_length);
    key_printer_.init(multi_attr_types[0], total_attr_length);
  }
...
RC BplusTreeHandler::open(const char *file_name)
{
  ...
  BufferPoolManager &bpm = BufferPoolManager::instance();
  FileBufferPool *disk_buffer_pool;
  ...
  header_dirty_ = false;
  file_buffer_pool_ = disk_buffer_pool;
  ...
  int total_attr_length = 0;
  for (auto length: file_header_.multi_attr_lengths) {
    total_attr_length += length;
  }

  if (file_header_.attr_amount > 1) {
    key_comparator_.init(CHARS, total_attr_length);
    key_printer_.init(CHARS, total_attr_length);
  } else {
    key_comparator_.init(file_header_.multi_attr_types[0], total_attr_length);
    key_printer_.init(file_header_.multi_attr_types[0], total_attr_length);
  }
...
RC BplusTreeHandler::print_internal_node_recursive(Frame *frame)
{
  ...
  for (int i = 0; i < node_size; i++) {
    ...
    Frame *child_frame;
    rc = file_buffer_pool_->get_this_page(page_num, &child_frame);
    if (rc != RC::SUCCESS) {
...
RC BplusTreeHandler::print_tree()
{
  ...
  RC rc = file_buffer_pool_->get_this_page(page_num, &frame);
  if (rc != RC::SUCCESS) {
...
bool BplusTreeHandler::validate_node_recursive(Frame *frame)
{
  ...
  if (node.is_leaf()) {
    LeafIndexNodeHandler leaf_node(file_header_, frame);
    result = leaf_node.validate(key_comparator_, file_buffer_pool_);
  } else {
    InternalIndexNodeHandler internal_node(file_header_, frame);
    result = internal_node.validate(key_comparator_, file_buffer_pool_);
    for (int i = 0; result && i < internal_node.size(); i++) {
      PageNum page_num = internal_node.value_at(i);
      Frame *child_frame;
      RC rc = file_buffer_pool_->get_this_page(page_num, &child_frame);
      if (rc != RC::SUCCESS) {
        LOG_WARN("failed to fetch child page.page id=%d, rc=%d:%s", page_num, rc, strrc(rc));
        result = false;
        break;
...
bool BplusTreeHandler::validate_tree()
{
  ...
  Frame *frame = nullptr;
  RC rc = file_buffer_pool_->get_this_page(file_header_.root_page, &frame);
  if (rc != RC::SUCCESS) {
...
RC BplusTreeHandler::find_leaf(BplusTreeOperationType op, const char *key, Frame *&frame)
{
    ...
    Frame *&frame)
{
  ...
  RC rc = crabing_protocal_fetch_page(op, file_header_.root_page, true/* is_root_node */, frame);
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to fetch root page. page id=%d, rc=%d:%s", file_header_.root_page, rc, strrc(rc));
    return rc;
  }
...
RC BplusTreeHandler::crabing_protocal_fetch_page(BplusTreeOperationType op,
                                                 PageNum page_num,
                                                 bool is_root_node,
                                                 Frame *&frame)
{
...
RC BplusTreeHandler::insert_entry_into_parent(Frame *frame, Frame *new_frame, const char *key)
{
  ...
  if (parent_page_num == BP_INVALID_PAGE_NUM) {
    ...
    Frame *root_frame;
    rc = file_buffer_pool_->allocate_page(&root_frame);
    if (rc != RC::SUCCESS) {
    ...
    file_buffer_pool_->unpin_page(frame);
    file_buffer_pool_->unpin_page(new_frame);

    update_root_page_num_locked(root_frame->page_num());
    root_frame->mark_dirty();
    file_buffer_pool_->unpin_page(root_frame);

    return RC::SUCCESS;
  } else {
    Frame *parent_frame = nullptr;
    rc = file_buffer_pool_->get_this_page(parent_page_num, &parent_frame);
    if (rc != RC::SUCCESS) {
    ...
    if (parent_node.size() < parent_node.max_size()) {
      ...
      file_buffer_pool_->unpin_page(frame);
      file_buffer_pool_->unpin_page(new_frame);
      file_buffer_pool_->unpin_page(parent_frame);
...
RC BplusTreeHandler::split(Frame *frame, Frame *&new_frame)
{
  ...
  // add a new node
  RC rc = file_buffer_pool_->allocate_page(&new_frame);
  if (rc != RC::SUCCESS) {
...
RC BplusTreeHandler::create_new_tree(const char *key, const RID *rid)
{
  ...
  Frame *frame = nullptr;
  rc = file_buffer_pool_->allocate_page(&frame);
  if (rc != RC::SUCCESS) {
...
MemPoolItem::unique_ptr BplusTreeHandler::make_key(const char *multi_keys[], const RID &rid, int multi_keys_amount, int left_or_right,  bool all_in_one_input_key)
{
  ...
  if (file_header_.attr_amount == 1) {
    memcpy(static_cast<char *>(key.get()), multi_keys[0], file_header_.multi_attr_lengths[0]);
    if (left_or_right != 3) {
      memcpy(static_cast<char *>(key.get()) + file_header_.multi_attr_lengths[0], &rid, sizeof(rid));
    }
  } else {
    if (left_or_right == 0) {  // occurs when inserting
      int accumulated_length = 0;
      for (int i=0; i < multi_keys_amount; i++) {
        memcpy(static_cast<char *>(key.get()) + accumulated_length, multi_keys[i], file_header_.multi_attr_lengths[i]);
        accumulated_length += file_header_.multi_attr_lengths[i];
      }
      memcpy(static_cast<char *>(key.get()) + accumulated_length, &rid, sizeof(rid));
    } else if (left_or_right == 1) {  // occurs when finding left_bound
      int accumulated_length = 0;
      if (all_in_one_input_key) {
        memcpy(static_cast<char *>(key.get()), multi_keys[0], file_header_.attrs_length);
        accumulated_length = file_header_.attrs_length;
      } else {
        memcpy(static_cast<char *>(key.get()), multi_keys[0], file_header_.multi_attr_lengths[0]);
        accumulated_length = file_header_.multi_attr_lengths[0];
        for (int i = 1; i < multi_keys_amount; i++) {
          memset(static_cast<char *>(key.get()) + accumulated_length, 0, file_header_.multi_attr_lengths[i]);
          accumulated_length += file_header_.multi_attr_lengths[i];
        }
      }
      memcpy(static_cast<char *>(key.get()) + accumulated_length, &rid, sizeof(rid));
    } else if (left_or_right == 2) {  // occurs when finding right_bound
      int accumulated_length = 0;
      if (all_in_one_input_key) {
        memcpy(static_cast<char *>(key.get()), multi_keys[0], file_header_.attrs_length);
        accumulated_length = file_header_.attrs_length;
...
{
  RID invalid_rid;
  MemPoolItem::unique_ptr pkey = make_key(multi_keys, invalid_rid, multi_keys_amount, 3);
  if (pkey == nullptr) {
    LOG_WARN("Failed to alloc memory for key.");
    return RC::NOMEM;
  }
  char *user_key = static_cast<char *>(pkey.get());
  BplusTreeScanner scanner(*this);
  RC rc = scanner.open(user_key, file_header_.attrs_length, true /*left_inclusive*/, user_key, file_header_.attrs_length, true /*right_inclusive*/);
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to open scanner. rc=%s", strrc(rc));
    return rc;
  }
  RID rid;
  while ((rc = scanner.next_entry(rid, false)) == RC::SUCCESS) {
    rids.push_back(rid);
  }
...
RC BplusTreeHandler::adjust_root(Frame *root_frame)
{
  ...
  if (root_node.is_leaf()) {
    ...
    RC rc = file_buffer_pool_->get_this_page(child_page_num, &child_frame);
    if (rc != RC::SUCCESS) {
    ...
    file_buffer_pool_->unpin_page(child_frame);
  ...
  file_buffer_pool_->unpin_page(root_frame);
  file_buffer_pool_->dispose_page(old_root_page_num);
...
RC BplusTreeHandler::coalesce_or_redistribute(Frame *frame)
{
  ...
  if (BP_INVALID_PAGE_NUM == parent_page_num) {
    ...
    if (index_node.size() > 1) {
      // root page has more than one child, no need to adjust
      file_buffer_pool_->unpin_page(frame);
      return RC::SUCCESS;
    } else {
      // adjust the root node
      return adjust_root(frame);
    }
  }

  Frame *parent_frame = nullptr;
  RC rc = file_buffer_pool_->get_this_page(parent_page_num, &parent_frame);
  if (rc != RC::SUCCESS) {
  ...
  rc = file_buffer_pool_->get_this_page(neighbor_page_num, &neighbor_frame);
  if (rc != RC::SUCCESS) {
...
RC BplusTreeHandler::coalesce(Frame *neighbor_frame, Frame *frame, Frame *parent_frame, int index)
{
  ...
  // parent_node.validate(key_comparator_, file_buffer_pool_, file_id_);
  RC rc = right_node.move_to(left_node, file_buffer_pool_);
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to move right node to left. rc=%d:%s", rc, strrc(rc));
    file_buffer_pool_->unpin_page(left_frame);
    file_buffer_pool_->unpin_page(right_frame);
    file_buffer_pool_->unpin_page(parent_frame);
    return rc;
  }
  ...
  left_frame->mark_dirty();
  parent_frame->mark_dirty();

  file_buffer_pool_->unpin_page(left_frame);
  file_buffer_pool_->unpin_page(right_frame);
  file_buffer_pool_->dispose_page(right_frame->page_num());
...
RC BplusTreeHandler::redistribute(Frame *neighbor_frame, Frame *frame, Frame *parent_frame, int index)
{
  ...
  if (index == 0) {
    ...
    neighbor_node.move_first_to_end(node, file_buffer_pool_);
    // neighbor_node.validate(key_comparator_, file_buffer_pool_, file_id_);
    // node.validate(key_comparator_, file_buffer_pool_, file_id_);
    parent_node.set_key_at(index + 1, neighbor_node.key_at(0));
    // parent_node.validate(key_comparator_, file_buffer_pool_, file_id_);
  } else {
    // the neighbor is at left
    neighbor_node.move_last_to_front(node, file_buffer_pool_);
    // neighbor_node.validate(key_comparator_, file_buffer_pool_, file_id_);
    // node.validate(key_comparator_, file_buffer_pool_, file_id_);
  ...
  file_buffer_pool_->unpin_page(neighbor_frame);
  file_buffer_pool_->unpin_page(frame);
  file_buffer_pool_->unpin_page(parent_frame);
...
RC BplusTreeHandler::delete_entry_internal(Frame *leaf_frame, const char *key)
{
  ...
  if (remove_count == 0) {
    ...
    file_buffer_pool_->unpin_page(leaf_frame);
    return RC::RECORD_NOT_EXIST;
  }
  // leaf_index_node.validate(key_comparator_, file_buffer_pool_, file_id_);
...
RC BplusTreeHandler::delete_entry(const char *multi_keys[], const RID *rid, int multi_keys_amount)
{
  MemPoolItem::unique_ptr pkey = make_key(multi_keys, *rid, multi_keys_amount);
  if (pkey == nullptr) {
    LOG_WARN("Failed to alloc memory for key.");
  ...
  RC rc = find_leaf(op, key, leaf_frame);
  if (rc == RC::EMPTY) {
  ...
  return delete_entry_internal(leaf_frame, key);
}
                          ...
                          const char *right_user_key, int right_len, bool right_inclusive)
{
  ...
  if (nullptr == left_user_key) {
    ...
    if (tree_handler_.file_header_.attrs_type == CHARS) {
    ...
    const char *multi_fixed_left_key[1] = {fixed_left_key};
    if (left_inclusive) {
      left_pkey = tree_handler_.make_key(multi_fixed_left_key, *RID::min(), tree_handler_.file_header_.attr_amount, 1, all_in_one_key_left);
    } else {
      left_pkey = tree_handler_.make_key(multi_fixed_left_key, *RID::max(), tree_handler_.file_header_.attr_amount, 2, all_in_one_key_left);
    }
    ...
    rc = tree_handler_.find_leaf(BplusTreeOperationType::READ, left_key, current_frame_);
    if (rc == RC::EMPTY) {
      rc = RC::SUCCESS;
  ...
  if (nullptr == right_user_key) {
    ...
    const char *multi_fixed_right_key[1] = {fixed_right_key};
    if (right_inclusive) {
      right_key_ = tree_handler_.make_key(multi_fixed_right_key, *RID::max(), tree_handler_.file_header_.attr_amount, 2, all_in_one_key_right);
    } else {
      right_key_ = tree_handler_.make_key(multi_fixed_right_key, *RID::min(), tree_handler_.file_header_.attr_amount, 1, all_in_one_key_right);
    }
...
RC BplusTreeScanner::next_entry(RID &rid, bool isdelete)
{
  ...
  iter_index_ = 0;
  return next_entry(rid, isdelete);
}
...
RC BplusTreeScanner::fix_user_key(const char *user_key, int key_len, bool want_greater, char **fixed_key, bool *should_inclusive)
{
  ...
  *should_inclusive = false;
  ...
  int32_t attr_length = tree_handler_.file_header_.attrs_length;
