  ...
  for (auto e : tokens) {
    ...
    if (nullptr == field) {
      LOG_ERROR("Deserialize index [%s]: no such field: %s",
                name_value.asCString(), e.c_str());
      return RC::SCHEMA_FIELD_MISSING;
    }
    multi_fields.emplace_back(field);
  }
  return index.init(unique_value.asBool(), name_value.asCString(),
                    multi_fields);
}

const char *IndexMeta::name() const { return name_.c_str(); }
const char *IndexMeta::field(int i) const { return multi_fields_[i].c_str(); }
...
const size_t IndexMeta::field_amount() const { return multi_fields_.size(); }
