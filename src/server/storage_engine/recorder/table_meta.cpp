...
RC TableMeta::init(int32_t table_id, const char *name, int field_num, const AttrInfoSqlNode attributes[])
{
  ...
  int null_field_len = (field_num + trx_field_num - 1) / 8 + 1;  // the length contain sys fields
  rc = fields_[field_num + trx_field_num].init("null_field", CHARS, field_offset, null_field_len, false, false);
  if (RC::SUCCESS != rc) {
    LOG_ERROR("Failed to init field meta. table name=%s, field name: %s", name, "null_field");
    return rc;
  }
...
RC TableMeta::init(int32_t table_id,
  ...
  LOG_INFO("Successfully initialized view meta. view id=%d, name=%s", table_id, name);
  return RC::SUCCESS;
}
...
int TableMeta::null_filed_num() const
{
  return 1;
}
