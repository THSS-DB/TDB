...
RC TableMeta::init(int32_t table_id, const char *name, int field_num,
  ...
  if (field_num <= 0 || nullptr == attributes) {
    LOG_ERROR("Invalid argument. name=%s, field_num=%d, attributes=%p", name,
              field_num, attributes);
    return RC::INVALID_ARGUMENT;
  }
  RC rc = RC::SUCCESS;
  ...
  const vector<FieldMeta> *trx_fields = TrxManager::instance()->trx_fields();
  if (trx_fields != nullptr) {
    fields_.resize(field_num + trx_fields->size() + null_filed_num);
    for (size_t i = 0; i < trx_fields->size(); i++) {
      const FieldMeta &field_meta = (*trx_fields)[i];
      fields_[i] = FieldMeta(field_meta.name(), field_meta.type(), field_offset,
                             field_meta.len(), false /*visible*/);
      field_offset += field_meta.len();
    }
    trx_field_num = static_cast<int>(trx_fields->size());
  } else {
    fields_.resize(field_num + null_filed_num);
  }
  ...
  for (int i = 0; i < field_num; i++) {
    ...
    rc = fields_[i + trx_field_num].init(attr_info.name.c_str(), attr_info.type,
                                         ...
                                         attr_info.nullable, true /*visible*/);
    if (rc != RC::SUCCESS) {
  ...
  int null_field_len =
      (field_num + trx_field_num - 1) / 8 + 1;  // the length contain sys fields
  rc = fields_[field_num + trx_field_num].init(
      "null_field", CHARS, field_offset, null_field_len, false, false);
  if (RC::SUCCESS != rc) {
  ...
  table_id_ = table_id;
  name_ = name;
  LOG_INFO("Successfully initialized table meta. table id=%d, name=%s",
           table_id, name);
  return RC::SUCCESS;
}

RC TableMeta::init(int32_t table_id, const char *name,
                   const char *origin_table_name, SelectStmt *select_stmt,
                   int field_num, const AttrInfoSqlNode attributes[]) {
  if (common::is_blank(name)) {
    LOG_ERROR("Name cannot be empty");
    return RC::INVALID_ARGUMENT;
  }
  ...
  table_id_ = table_id;
  name_ = name;
  origin_table_name_ = origin_table_name;
  is_view_ = true;
  select_stmt_ = select_stmt;
  LOG_INFO("Successfully initialized view meta. view id=%d, name=%s", table_id,
           name);
  return RC::SUCCESS;
}
...
int TableMeta::null_filed_num() const { return 1; }
...
int TableMeta::deserialize(std::istream &is) {
  ...
  if (!table_id_value.isInt()) {
    LOG_ERROR("Invalid table id. json value=%s",
              table_id_value.toStyledString().c_str());
    return -1;
  }
