...
FileBufferPool::FileBufferPool(BufferPoolManager &bp_manager,
...
RC FileBufferPool::open_file(const char *file_name) {
  ...
  if (rc != RC::SUCCESS) {
    LOG_ERROR("failed to allocate frame for header. file name %s",
              file_name_.c_str());
    close(fd);
    file_desc_ = -1;
    return rc;
  ...
  LOG_INFO("Successfully open %s. file_desc=%d, hdr_frame=%p, file header=%s",
           file_name, file_desc_, hdr_frame_,
...
RC FileBufferPool::close_file() {
  ...
  rc = evict_all_pages();
  if (rc != RC::SUCCESS) {
    LOG_ERROR("failed to close %s, due to failed to purge pages. rc=%s",
              file_name_.c_str(), strrc(rc));
    return rc;
  }
  ...
  if (close(file_desc_) < 0) {
    LOG_ERROR("Failed to close fileId:%d, fileName:%s, error:%s", file_desc_,
...
RC FileBufferPool::get_this_page(PageNum page_num, Frame **frame) {
  ...
  *frame = nullptr;
  ...
  std::scoped_lock lock_guard(
      lock_);  // 直接加了一把大锁，其实可以根据访问的页面来细化提高并行度
  ...
  if (rc != RC::SUCCESS) {
    LOG_ERROR("Failed to alloc frame %s:%d, due to failed to alloc page.",
...
RC FileBufferPool::allocate_page(Frame **frame) {
  ...
  if ((rc = allocate_frame(page_num, &allocated_frame)) != RC::SUCCESS) {
    LOG_ERROR("Failed to allocate frame %s, due to no free page.",
              file_name_.c_str());
    lock_.unlock();
    return rc;
  }

  LOG_INFO("allocate new page. file=%s, pageNum=%d, pin=%d", file_name_.c_str(),
           page_num, allocated_frame->pin_count());
 ...
 */
RC FileBufferPool::evict_all_pages() { return RC::SUCCESS; }
...
RC FileBufferPool::allocate_frame(PageNum page_num, Frame **buffer) {
  auto evict_action = [this](Frame *frame) {
    ...
    if (rc != RC::SUCCESS) {
      LOG_ERROR(
          "Failed to aclloc block due to failed to flush old block. rc=%s",
          strrc(rc));
    }
    return rc;
  };
  ...
  while (true) {
    ...
    LOG_TRACE(
        "frames are all allocated, so we should evict some frames to get one "
        "free frame");
...
RC FileBufferPool::load_page(PageNum page_num, Frame *frame) {
  ...
  if (ret != 0) {
    LOG_ERROR(
        "Failed to load page %s, file_desc:%d, page num:%d, due to failed to "
        "read data:%s, ret=%d, page count=%d",
        file_name_.c_str(), file_desc_, page_num, strerror(errno), ret,
        file_header_->allocated_pages);
...
int FileBufferPool::file_desc() const { return file_desc_; }

RC FileBufferPool::recover_page(PageNum page_num) { return RC::SUCCESS; }
...
BufferPoolManager::BufferPoolManager(int memory_size /* = 0 */) {
  ...
  const int pool_num =
      std::max(memory_size / BP_PAGE_SIZE / DEFAULT_ITEM_NUM_PER_POOL, 1);
  frame_manager_.init(pool_num);
  LOG_INFO(
      "buffer pool manager init with memory size %d, page num: %d, pool num: "
      "%d",
...
RC BufferPoolManager::open_file(const char *_file_name, FileBufferPool *&_bp) {
  ...
  fd_buffer_pools_.insert(
      std::pair<int, FileBufferPool *>(bp->file_desc(), bp));
  LOG_DEBUG("insert buffer pool into fd buffer pools. fd=%d, bp=%p, lbt=%s",
            bp->file_desc(), bp, lbt());
  _bp = bp;
  return RC::SUCCESS;
}
...
RC BufferPoolManager::close_file(const char *_file_name) {
  ...
  if (0 == fd_buffer_pools_.erase(fd)) {
    int count = 0;
    for (auto fd_iter = fd_buffer_pools_.begin();
         fd_iter != fd_buffer_pools_.end(); ++fd_iter) {
 ...
 */
RC BufferPoolManager::flush_page(Frame &frame) { return RC::SUCCESS; }
