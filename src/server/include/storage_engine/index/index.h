...
class Index {
 public:
  ...
  virtual RC create(const char *file_name, const IndexMeta &index_meta,
                    const std::vector<FieldMeta> &multi_field_metas);
  virtual RC open(const char *file_name, const IndexMeta &index_meta,
                  const std::vector<FieldMeta> &multi_field_metas);
  virtual RC close();
  ...
  virtual RC insert_entry(const char *record, const RID *rid, int sys_field_num,
                          MvccTrx *trx);
  /**
   * @brief 删除一条数据
   *
