 ...
 * RecordManager本身没有再单独拿一个页面来存放元数据，每一个页面都存放了一个页面头信息，也就是每个页面都有
 * RecordManager 的元数据信息。
 * 可以参考PageHeader，这虽然有点浪费但是做起来简单。
 *
 * 对单个页面来说，最开始是一个页头，然后接着就是一行行记录（会对齐）。
 * 如何标识一个记录，或者定位一个记录？
 * 使用RID，即record identifier。使用 page num 表示所在的页面，slot num
 * 表示当前在页面中的位置。 因为这里的记录都是定长的，所以根据slot num
 * 可以直接计算出记录的起始位置。 问题1：那么如果记录不是定长的，还能使用 slot
 * num 吗？ 问题2：如何更有效地存放不定长数据呢？
...
class RecordPageHandler {
 public:
   ...
   */
  RC init_empty_page(FileBufferPool &buffer_pool, PageNum page_num,
                     int record_size);
   ...
   * @param rid 指定的位置
   * @param rec
   * 返回指定的数据。这里不会将数据复制出来，而是使用指针，所以调用者必须保证数据使用期间受到保护
   */
 ...
 protected:
  ...
  char *get_record_data(SlotNum slot_num) {
    return frame_->data() + page_header_->first_record_offset +
...
class RecordFileHandler {
 public:
   ...
   * @note rec 参数返回的记录并不会复制数据内存。page_handler
   * 对象会拿着相关的资源，比如 pin 住页面和加上页面锁。 如果page_handler
   * 释放了，那也不能再访问rec对象了。
   */
  RC get_record(RecordPageHandler &page_handler, const RID *rid, bool readonly,
                Record *rec);
 ...
 private:
  ...
  common::Mutex lock_;  // 未满page集合free_pages_的锁。当编译时增加-DCONCURRENCY=ON
                        // 选项时，才会真正的支持并发
};
...
class RecordFileScanner {
 public:
   ...
   */
  RC open_scan(Table *table, FileBufferPool &buffer_pool, Trx *trx,
               bool readonly, ConditionFilter *condition_filter);
