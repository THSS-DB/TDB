...
struct RID {
  ...
  /**
   * 返回一个不可能出现的最小的RID
   * 虽然page num 0和slot num 0都是合法的，但是page num
   * 0通常用于存放meta数据，所以对数据部分来说都是不合法的. 这里在bplus
   * tree中查找时会用到。
   */
  static RID *min() {
    static RID rid{0, 0};
   ...
   */
  static RID *max() {
    static RID rid{std::numeric_limits<PageNum>::max(),
                   std::numeric_limits<SlotNum>::max()};
    return &rid;
  }
};
