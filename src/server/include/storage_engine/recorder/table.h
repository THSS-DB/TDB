...
class Table {
 public:
   ...
   */
  RC create(int32_t table_id, const char *name, const char *origin_table_name,
   ...
   * @param base_dir 表数据存放的路径
   */
  RC drop(int32_t table_id, const char *name, const char *base_dir);
  ...
  RC create_index(Trx *trx, std::vector<const FieldMeta *> &multi_field_metas,
 ...
 public:
  ...
  const char *origin_table_name() const {
    return table_meta_.origin_table_name();
  }
 ...
 private:
  ...
  //  RC insert_entry_of_indexes(const char *record, const RID &rid, MvccTrx
  //  *trx);
