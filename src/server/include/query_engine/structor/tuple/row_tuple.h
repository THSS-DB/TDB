...
class RowTuple : public Tuple {
 public:
  ...
  void _set_record(Record *record) {
    ...
    this->bitmap_.init(record->data() + null_filed_meta->offset(),
                       null_filed_meta->len());
  }

  void set_schema(const Table *table, const std::string &table_alias,
  ...
  int cell_num() const override { return species_.size(); }

  void remove_order_set() { order_set_ = false; }

  bool order_set() const { return order_set_; }
