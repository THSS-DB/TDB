...
class AggrTuple : public Tuple {
 public:
  ...
  int cell_num() const override { return aggr_results_.size(); }

  void set_tuple(std::vector<std::string> &alias,
                 std::vector<Value> &aggr_results) {
    for (auto &alia : alias) {
  ...
  RC find_cell(const TupleCellSpec &spec, Value &cell) const override {
    for (int i = 0; i < alias_.size(); i++) {
      if (strcmp(alias_[i].c_str(), spec.alias()) == 0 ||
          strcmp(alias_[i].c_str(), spec.field_name()) == 0) {
